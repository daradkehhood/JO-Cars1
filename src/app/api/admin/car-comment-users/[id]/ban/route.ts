...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (duration === undefined || duration === null) return Response.json({ error: 'المدة مطلوبة' }, { status: 400 });
  ...
  const updatedUser = await prisma.user.update({
    where: { id },
    data: { carCommentBannedUntil: banUntil },
    select: { id: true, name: true, carCommentBannedUntil: true },
  });
